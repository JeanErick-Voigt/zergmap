...
void tree_insert(BST *root, double longitude, double altitude, double latitude, int id);
BST * init_tree(double longitude, double altitude, double latitude, int id);
...
//Indent all the information below this line for while loop
	...
	while(ftell(fp) < fullSize)
	{
		...
		if(root != NULL){
			printf("This is root id in beginning %d\n", root->id);
		}
		count++;
	...
	//	printf("This is the old fp pointer %d\n", ftell(fp));		
		...
		//int len = ftell(fp);
		printf("This is sizeof ethernet %d\n", sizeof(ether));
		
		int ipVersion;
		uint8_t version;
		printf("This is fp before reading version %ld\n", ftell(fp));
		fread(&version, sizeof(version), 1, fp);
		printf("This is is fp after reading version %ld\n", ftell(fp));
		ipVersion = version >> 4;
		...
		if(ipVersion != 4){
			...
			//printf("this is ipv4\n");
			//printf("this is ftell before reading fp %ld\n", ftell(fp));
			//printf("This is sizeof ip struct %d\n", sizeof(internetProtocol));
			fread(&internetProtocol, sizeof(internetProtocol), 1, fp);
			fseek(fp, -1, SEEK_CUR);
			//printf("this is file pointer after substraction %d\n", ftell(fp));
			int ipLength = NTOH2(internetProtocol.iptotalLength);
			//printf("This is ipLength %d\n", ipLength);
			//printf("After reading ip version %ld\n", ftell(fp));
		}
		//IPV4 ip;
		//int ihl;
		//uint8_t mask = 1;
		...
		//ihl = ip.versionAndIHL & 0x0F;  may use later for IHL data for ipv4
		...
		printf("Sequence : %u\n", sequence);
		if(root != NULL){
			printf("This is root id after sequence %d\n", root->id);
		}
		printf("From : %d\n", zergSourceID);
		
		if(root != NULL){
			printf("This is root id after print zergsource %d\n", root->id);
		}
		printf("To : %d\n", zergDestinationID);

		if(root != NULL){
			printf("This is root middle of status payload %d\n", root->id);
		}
		...
		if(root != NULL){
			printf("This is root after message payload %d\n", root->id);
		}
		if(root != NULL){
			printf("This is  root after message payload malloc %d\n", root->id);
		}
		//messagePayload[messageLength] = '\0';
		if(root != NULL){
			printf("This is root id before command payload %d\n", root->id);
		}
		COMMAND command;
		uint16_t parameter1;
		...
		GPS gpsDataPayload;
// declaration of doubles for zerg
	...
	//	int space = 2;
		...
		//BST *root = NULL;
		if (root == NULL){
			root = init_tree(newLong, alt32, newLat, zergSourceID);
		...
		tree_insert(root, newLong, alt32, newLat, zergSourceID);
		printf("This is beginning of inorder statement\n\n");
		printf("root left %d root right %d\n", root->left->id, root->right->id);
		//inorder(root);

	}
	//printf("This is final inorder statement\n");
	//inorder(root);
...
void tree_insert(BST *root, double longitude, double altitude, double latitude, int id)
{
	...
	printf("ID is less\n");
		if(root->left != NULL){
			tree_insert(root->left, longitude, altitude, latitude, id);
		}else{
			root->left = init_tree(longitude, altitude, latitude, id);
			printf("this is id %d\n", root->left->id);
		}
	}
	else if(id > root->id){
	printf("Id is more\n");
		if(root->right != NULL){
			tree_insert(root->right, longitude, altitude, latitude, id);
		}else{
			root->right = init_tree(longitude, altitude, latitude, id);
			printf("This is id %d\n", root->right->id);
		}
	}else{
		return;
